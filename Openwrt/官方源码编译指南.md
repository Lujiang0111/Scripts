...
+ (24.10及以下)`fakeroot`：按[master](https://github.com/openwrt/openwrt/tree/main/tools/fakeroot)修改
...
+ (23.05及以下)`gnulib`：按[24.10](https://github.com/openwrt/openwrt/tree/openwrt-24.10/tools/gnulib)修改

  ```shell
  rm -rf tools/gnulib/
  git clone -b openwrt-24.10 --depth=1 --no-checkout https://github.com/openwrt/openwrt.git tools/gnulib/clone
  cd tools/gnulib/clone/
  git sparse-checkout init --cone
  git sparse-checkout set tools/gnulib
  git checkout
  cd -
  mv tools/gnulib/clone/tools/gnulib/* tools/gnulib/
  rm -rf tools/gnulib/clone/
  ```
